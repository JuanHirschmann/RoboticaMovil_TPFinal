...
SIMULATION_DURATION = 1*60;%3*60;          % Duracion total [s]
SAMPLE_TIME = 0.1;                   % Sample time [s]
INIT_POS = [4,2,0];%random_empty_point(MAP,[5,1],[5,1]);%[2.5; 1.5; -pi/2];         % Pose inicial (x y theta) del robot simulado (el robot pude arrancar en cualquier lugar valido del mapa)
...
%aaa = findpath(planner,start,xyA(2,:));

[velA,thetaA] = velocity_model(xyA,theta,SAMPLE_TIME);
[velB,thetaB] = velocity_model(xyB,thetaA,SAMPLE_TIME);
